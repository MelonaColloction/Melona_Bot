...
# Added: Admin /userinfo command, winner history tracking, pending withdrawal prevention
...
class Database:
    ...
    def connect(self):
        ...
        if not database_url:
            print("❌ DATABASE_URL not set! Please add PostgreSQL database in Railway.")
            sys.exit(1)
    ...
    def init_tables(self):
        self.execute("""
            CREATE TABLE IF NOT EXISTS users (
                ...
                is_marketing_admin BOOLEAN DEFAULT FALSE
            )
        """, commit=True)
        self.execute("""
    ...
    # ----- User Operations (با placeholder %s به جای ?) -----
    def register_marketing_admin_if_needed(self, user_id: int, username: str):
        if user_id == MARKETING_ADMIN_ID:
            self.execute("INSERT INTO users (user_id, username, registered_at, is_verified, is_marketing_admin, tokens, level) VALUES (%s, %s, %s, %s, %s, %s, %s) ON CONFLICT (user_id) DO NOTHING",
                        (user_id, username, datetime.now(), True, True, 0, '👑 Admin'), commit=True)
            return True
        return False
    
    def register_user(self, user_id: int, username: str, tx_hash: str, network: str, invited_by: int = None):
        self.execute("INSERT INTO users (user_id, username, registered_at, invited_by, payment_tx_hash, payment_network, is_verified, tokens, level) VALUES (%s, %s, %s, %s, %s, %s, %s, %s, %s) ON CONFLICT (user_id) DO UPDATE SET username = EXCLUDED.username",
                    (user_id, username, datetime.now(), invited_by, tx_hash, network, True, 0, '🐣 Newbie'), commit=True)
    
    def update_user_level(self, user_id: int, referral_count: int):
        if referral_count >= 1000: level = "👑 Diamond"
        elif referral_count >= 500: level = "💎 Platinum"
        elif referral_count >= 100: level = "🥇 Gold"
        elif referral_count >= 50: level = "🥈 Silver"
        elif referral_count >= 10: level = "🥉 Bronze"
        else: level = "🐣 Newbie"
        self.execute("UPDATE users SET level = %s WHERE user_id = %s", (level, user_id), commit=True)
    
    def set_user_pin(self, user_id: int, pin: str):
        hashed_pin, salt = hash_pin(pin)
        self.execute("UPDATE users SET user_pin = %s, pin_salt = %s, pin_set_at = %s WHERE user_id = %s", (hashed_pin, salt, datetime.now(), user_id), commit=True)
    
    def verify_user_pin(self, user_id: int, pin: str) -> bool:
        row = self.fetchone("SELECT user_pin, pin_salt FROM users WHERE user_id = %s", (user_id,))
        if not row or not row[0]:
            return False
        return verify_pin(pin, row[0], row[1])
    
    def has_pin(self, user_id: int) -> bool:
        row = self.fetchone("SELECT user_pin FROM users WHERE user_id = %s", (user_id,))
        return row and row[0] is not None
    
    def reset_pin(self, user_id: int):
        self.execute("UPDATE users SET user_pin = NULL, pin_salt = NULL, pin_set_at = NULL WHERE user_id = %s", (user_id,), commit=True)
    
    def get_referrer(self, user_id: int):
        row = self.fetchone("SELECT invited_by FROM users WHERE user_id = %s", (user_id,))
        return row[0] if row else None
    
    def add_balance(self, user_id: int, amount: float):
        self.execute("UPDATE users SET balance = balance + %s WHERE user_id = %s", (amount, user_id), commit=True)
    
    def add_tokens_for_referral(self, user_id: int):
        self.execute("UPDATE users SET tokens = tokens + %s WHERE user_id = %s", (TOKENS_PER_REFERRAL, user_id), commit=True)
    
    def add_tokens(self, user_id: int, amount: int):
        self.execute("UPDATE users SET tokens = tokens + %s WHERE user_id = %s", (amount, user_id), commit=True)
    
    def get_balance(self, user_id: int) -> float:
        row = self.fetchone("SELECT balance FROM users WHERE user_id = %s", (user_id,))
        return row[0] if row else 0.0
    
    def get_tokens(self, user_id: int) -> int:
        row = self.fetchone("SELECT tokens FROM users WHERE user_id = %s", (user_id,))
        return row[0] if row else 0
    
    def deduct_tokens(self, user_id: int, amount: int) -> bool:
        cur = self.execute("UPDATE users SET tokens = tokens - %s WHERE user_id = %s AND tokens >= %s RETURNING user_id", (amount, user_id, amount), commit=True)
        return len(cur) > 0 if cur else False
    
    def set_wallet_address(self, user_id: int, address: str, network: str):
        self.execute("UPDATE users SET wallet_address = %s, wallet_network = %s WHERE user_id = %s", (address, network, user_id), commit=True)
    
    def set_pending_wallet_address(self, user_id: int, address: str, network: str):
        self.execute("UPDATE users SET pending_wallet_address = %s, pending_wallet_network = %s WHERE user_id = %s", (address, network, user_id), commit=True)
    
    def get_pending_wallet_address(self, user_id: int):
        return self.fetchone("SELECT pending_wallet_address, pending_wallet_network FROM users WHERE user_id = %s", (user_id,))
    
    def clear_pending_wallet(self, user_id: int):
        self.execute("UPDATE users SET pending_wallet_address = NULL, pending_wallet_network = NULL WHERE user_id = %s", (user_id,), commit=True)
    
    def apply_pending_wallet(self, user_id: int) -> bool:
        row = self.fetchone("SELECT pending_wallet_address, pending_wallet_network FROM users WHERE user_id = %s", (user_id,))
        if row and row[0]:
            self.execute("UPDATE users SET wallet_address = %s, wallet_network = %s, pending_wallet_address = NULL, pending_wallet_network = NULL WHERE user_id = %s", (row[0], row[1], user_id), commit=True)
            return True
        return False
    
    def set_ton_wallet_address(self, user_id: int, address: str):
        self.execute("UPDATE users SET ton_wallet_address = %s WHERE user_id = %s", (address, user_id), commit=True)
    
    def get_ton_wallet_address(self, user_id: int):
        row = self.fetchone("SELECT ton_wallet_address FROM users WHERE user_id = %s", (user_id,))
        return row[0] if row else None
    
    def get_user_wallet(self, user_id: int):
        return self.fetchone("SELECT wallet_address, wallet_network FROM users WHERE user_id = %s", (user_id,))
    
    def add_withdrawal_request(self, user_id: int, amount: float, address: str, network: str) -> int:
        cur = self.execute("INSERT INTO withdrawals (user_id, amount, wallet_address, wallet_network, requested_at) VALUES (%s, %s, %s, %s, %s) RETURNING id", (user_id, amount, address, network, datetime.now()), commit=True)
        return cur[0][0]
    
    def add_token_withdrawal_request(self, user_id: int, amount: int, address: str) -> int:
        cur = self.execute("INSERT INTO token_withdrawals (user_id, amount, ton_wallet_address, requested_at) VALUES (%s, %s, %s, %s) RETURNING id", (user_id, amount, address, datetime.now()), commit=True)
        return cur[0][0]
    
    def get_withdrawal(self, request_id: int):
        return self.fetchone("SELECT user_id, amount, wallet_address, wallet_network, status FROM withdrawals WHERE id = %s", (request_id,))
    
    def get_token_withdrawal(self, request_id: int):
        return self.fetchone("SELECT user_id, amount, ton_wallet_address, status FROM token_withdrawals WHERE id = %s", (request_id,))
    
    def update_withdrawal_status(self, request_id: int, status: str):
        self.execute("UPDATE withdrawals SET status = %s WHERE id = %s", (status, request_id), commit=True)
    
    def update_token_withdrawal_status(self, request_id: int, status: str):
        self.execute("UPDATE token_withdrawals SET status = %s WHERE id = %s", (status, request_id), commit=True)
    
    def deduct_balance(self, user_id: int, amount: float) -> bool:
        cur = self.execute("UPDATE users SET balance = balance - %s WHERE user_id = %s AND balance >= %s RETURNING user_id", (amount, user_id, amount), commit=True)
        return len(cur) > 0 if cur else False
    
    def get_pending_withdrawals(self):
        return self.fetchall("SELECT id, user_id, amount, wallet_address, wallet_network, requested_at FROM withdrawals WHERE status = 'pending' ORDER BY requested_at ASC")
    
    def get_pending_token_withdrawals(self):
        return self.fetchall("SELECT id, user_id, amount, ton_wallet_address, requested_at FROM token_withdrawals WHERE status = 'pending' ORDER BY requested_at ASC")
    
    def get_stats(self):
        total_users = self.fetchone("SELECT COUNT(*) FROM users")[0]
        total_balance = self.fetchone("SELECT COALESCE(SUM(balance), 0) FROM users")[0]
        total_withdrawn = self.fetchone("SELECT COALESCE(SUM(amount), 0) FROM withdrawals WHERE status = 'completed'")[0]
        pending_count = self.fetchone("SELECT COUNT(*) FROM withdrawals WHERE status = 'pending'")[0]
        total_tokens = self.fetchone("SELECT COALESCE(SUM(tokens), 0) FROM users")[0]
        return total_users, total_balance, total_withdrawn, pending_count, total_tokens
    
    def user_exists(self, user_id: int) -> bool:
        row = self.fetchone("SELECT 1 FROM users WHERE user_id = %s", (user_id,))
        return row is not None
    
    def get_user_info(self, user_id: int):
        return self.fetchone("SELECT username, balance, is_marketing_admin, tokens, ton_wallet_address, level FROM users WHERE user_id = %s", (user_id,))
    
    def get_referral_count(self, user_id: int) -> int:
        row = self.fetchone("SELECT COUNT(*) FROM users WHERE invited_by = %s", (user_id,))
        return row[0] if row else 0
    ...
    def increment_weekly_referral(self, user_id: int):
        ...
        self.execute("INSERT INTO weekly_referrals (user_id, week_start, count) VALUES (%s, %s, 1) ON CONFLICT (user_id) DO UPDATE SET count = weekly_referrals.count + 1", (user_id, week_start), commit=True)
    
    def get_weekly_leaderboard(self):
        today = datetime.now().date()
        week_start = today - timedelta(days=today.weekday())
        return self.fetchall("SELECT user_id, count FROM weekly_referrals WHERE week_start = %s ORDER BY count DESC LIMIT 10", (week_start,))
    
    def get_previous_week_winners(self):
        today = datetime.now().date()
        last_week_start = today - timedelta(days=today.weekday() + 7)
        return self.fetchall("SELECT user_id, count FROM weekly_referrals WHERE week_start = %s ORDER BY count DESC LIMIT 10", (last_week_start,))
    
    def reset_weekly_referrals(self):
        today = datetime.now().date()
        week_start = today - timedelta(days=today.weekday())
        self.execute("DELETE FROM weekly_referrals WHERE week_start < %s", (week_start,), commit=True)
    ...
    def has_pending_withdrawal(self, user_id: int) -> bool:
        row = self.fetchone("SELECT 1 FROM withdrawals WHERE user_id = %s AND status = 'pending'", (user_id,))
        return row is not None
    
    def has_pending_token_withdrawal(self, user_id: int) -> bool:
        row = self.fetchone("SELECT 1 FROM token_withdrawals WHERE user_id = %s AND status = 'pending'", (user_id,))
        return row is not None

    def record_airdrop_winner(self, user_id: int, amount: int):
        self.execute("INSERT INTO airdrop_winners (user_id, won_at, amount) VALUES (%s, %s, %s)", (user_id, datetime.now(), amount), commit=True)

    def get_airdrop_wins_count(self, user_id: int) -> int:
        row = self.fetchone("SELECT COUNT(*) FROM airdrop_winners WHERE user_id = %s", (user_id,))
        return row[0] if row else 0

    def record_leaderboard_winner(self, user_id: int, week_start: str, rank: int, reward: int):
        self.execute("INSERT INTO leaderboard_winners (user_id, week_start, rank, reward) VALUES (%s, %s, %s, %s)", (user_id, week_start, rank, reward), commit=True)

    def get_leaderboard_wins_count(self, user_id: int) -> int:
        row = self.fetchone("SELECT COUNT(*) FROM leaderboard_winners WHERE user_id = %s", (user_id,))
        return row[0] if row else 0

    def get_current_week_leaderboard_rank(self, user_id: int):
        today = datetime.now().date()
        week_start = today - timedelta(days=today.weekday())
        rows = self.fetchall("SELECT user_id, count FROM weekly_referrals WHERE week_start = %s ORDER BY count DESC", (week_start,))
...
# ============================================
# MY_STATS (اصلاح شده برای PostgreSQL)
# ============================================
@rate_limited
async def my_stats(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = db.fetchone("SELECT username, balance, tokens, (SELECT COUNT(*) FROM users WHERE invited_by = %s), level FROM users WHERE user_id = %s", (user_id, user_id))
    if not row:
        return
    
    username, balance, tokens, referral_count, level = row
    rank_row = db.fetchone("SELECT COUNT(*) + 1 FROM users u WHERE (SELECT COUNT(*) FROM users WHERE invited_by = u.user_id) > %s", (referral_count,))
    rank = rank_row[0] if rank_row and referral_count > 0 else "N/A"
...
async def admin_reset_pin(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    row = db.fetchone("SELECT user_id FROM users WHERE username = %s", (username,))
    if not row:
    ...
    try:
        await context.bot.send_message(chat_id=row[0], text="🔐 Your PIN has been reset by admin. Please set a new PIN.")
    except:
        pass
...
# ============================================
# ADMIN COMMAND: USERINFO (ONLY FOR FINANCE ADMIN)
# ============================================
@rate_limited
async def userinfo(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    reg_date = db.fetchone("SELECT registered_at FROM users WHERE user_id = %s", (target_id,))
    reg_date_str = reg_date[0] if reg_date else "Unknown"
    await update.message.reply_text(f"📋 **User Information**\n\n🆔 ID: `{target_id}`\n👤 Username: @{username or 'No username'}\n📅 Registered: {reg_date_str}\n🏅 Level: {level}\n🔐 PIN: {'✅ Set' if has_pin else '❌ Not set'}\n\n💰 USD Balance: {balance} USD\n🍯 HONEY Balance: {tokens}\n👥 Total Referrals: {referral_count}\n\n🏆 Leaderboard Wins: {leaderboard_wins}\n📊 Current Week Rank: {rank_text}\n🎁 Airdrop Wins: {airdrop_wins}\n\n💳 Withdrawal Wallet: {wallet_address} ({wallet_network})\n🪙 TON Wallet: {ton_wallet or 'Not set'}", parse_mode="Markdown")
...
def main():
    ...
    print("🍈 Melona Bot is running with PostgreSQL!")
